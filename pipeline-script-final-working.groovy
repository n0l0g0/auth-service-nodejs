...
EOF
    ...
    stage('Create DB Secret') {
      steps {
        script {
          sh """
            oc create secret generic auth-service-db-secret -n ${NAMESPACE} \\
              --from-literal=DB_HOST=188.166.254.121 \\
              --from-literal=DB_PORT=6543 \\
              --from-literal=DB_DATABASE=demo_db \\
    ...
    stage('Create JWT Secret') {
      steps {
        script {
          sh """
            oc create secret generic auth-service-secret -n ${NAMESPACE} \\
              --from-literal=JWT_SECRET=super_secret_jwt_key \\
              --dry-run=client -o yaml | oc apply -f -
          """
        }
      }
